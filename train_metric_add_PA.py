...
from models.fewshot_v5 import FewShotSeg
...
def main(_run, _config, _log):
    ...
    i_iter = 0
    log_loss = {'loss': 0, 'metric_loss': 0, 'PA_loss': 0, 'align_loss': 0}
    _log.info('###### Training ######')
    for i_iter, sample_batched in enumerate(trainloader):
        ...
        optimizer.zero_grad()
        supp_fg_fts, supp_bg_fts, qry_fts, query_pred, align_loss = model(support_images, support_fg_mask, support_bg_mask,
                                       query_images)
        ...
        metric_loss = loss_func(fts, label.cuda(), hard_pairs)

        # PA Loss
        query_loss = criterion(query_pred, query_labels)
        PA_loss = query_loss + align_loss * _config['align_loss_scaler']
        
        loss = metric_loss + PA_loss
        loss.backward()
        optimizer.step()
        scheduler.step()
        ...
        writer.add_scalar('Loss/metric_loss', metric_loss, i_iter)
        writer.add_scalar('Loss/PA_loss', PA_loss, i_iter)
        writer.add_scalar('Loss/align_loss', align_loss, i_iter)

        # Log loss
        loss = loss.detach().data.cpu().numpy()
        metric_loss = metric_loss.detach().data.cpu().numpy()
        PA_loss = PA_loss.detach().data.cpu().numpy()
        align_loss = align_loss.detach().data.cpu().numpy() if align_loss != 0 else 0
        _run.log_scalar('loss', loss)
        _run.log_scalar('metric_loss', metric_loss)
        _run.log_scalar('PA_loss', PA_loss)
        _run.log_scalar('align_loss', align_loss)
        log_loss['loss'] += loss
        log_loss['metric_loss'] += metric_loss
        log_loss['PA_loss'] += PA_loss
        ...
        if (i_iter + 1) % _config['print_interval'] == 0:
            ...
            metric_loss = log_loss['metric_loss'] / (i_iter + 1)
            PA_loss = log_loss['PA_loss'] / (i_iter + 1)
            align_loss = log_loss['align_loss'] / (i_iter + 1)
            print(f'step {i_iter+1}: loss: {loss}, metric_loss: {metric_loss}, PA_loss: {PA_loss}, align_loss: {align_loss}')
