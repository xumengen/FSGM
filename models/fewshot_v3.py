...
class FewShotSeg(nn.Module):
    ...
    def forward(self, supp_imgs, fore_mask, back_mask, qry_imgs):
        ...
        h_value = fore_mask.shape[-1]
        w_value = fore_mask.shape[-2]
        ...
        for epi in range(batch_size):
            ...
            fg_prototypes, bg_prototype = self.getPrototype_v2(supp_fg_fts, supp_bg_fts)  # Wa * Sh * [1 * C * H * W]

            ###### Compute the distance ######
            # prototypes = [bg_prototype,] + fg_prototypes
            # dist = [self.calDist(qry_fts[:, epi], prototype) for prototype in prototypes]
            max_dist_list = []
            max_dist = torch.full_like(qry_fts[:, epi, 0], 0)
            
            count = 0
            for way in range(n_ways):
                for shot in range(n_shots):
                    for h in range(h_value):
                        for w in range(w_value):
                            if back_mask[way, shot, [epi], h, w] == 1:
                                count += 1
                                # max_dist = torch.maximum(max_dist, self.calDist_v2(qry_fts[:, epi], bg_prototype[way][shot][:, :, h, w]))
                                max_dist += self.calDist_v2(qry_fts[:, epi], bg_prototype[way][shot][:, :, h, w])
            max_dist_list.append(max_dist/count)
            ...
            for way in range(n_ways):
                ...
                for shot in range(n_shots):
                    for h in range(h_value):
                        for w in range(w_value):
                            if fore_mask[way, shot, [epi], h, w] == 1:
                                count += 1
                                # max_dist = torch.maximum(max_dist, self.calDist_v2(qry_fts[:, epi], fg_prototypes[way][shot][:, :, h, w]))
                                max_dist += self.calDist_v2(qry_fts[:, epi], fg_prototypes[way][shot][:, :, h, w])
                max_dist_list.append(max_dist/(count))


            pred = torch.stack(max_dist_list, dim=1)  # N x (1 + Wa) x H' x W'
    ...
    def calDist_v2(self, fts, prototype, scaler=20):
        """
        Calculate the distance between features and prototypes
    ...
    def getFeatures_v2(self, fts, mask):
        ...
        fts = F.interpolate(fts, size=mask.shape[-2:], mode='bilinear')
        # masked_fts = torch.sum(fts * mask[None, ...], dim=(2, 3)) 
        # masked_fts = fts * mask[None, ...] # 1 * C * H * W
        return fts
    ...
    def getPrototype_v2(self, fg_fts, bg_fts):
        """
        Average the features to obtain the prototype

        Args:
            fg_fts: lists of list of foreground features for each way/shot
                expect shape: Wa * Sh * [1 * C * H * W]
            bg_fts: lists of list of background features for each way/shot
                expect shape: Wa * Sh * [1 * C * H * W]
        """
        # n_ways, n_shots = len(fg_fts), len(fg_fts[0])
